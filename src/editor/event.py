...
class CallArgs(object):
    ...
    def __add__(self, other):
        duplicates = set(other.kwargs) & set(self.kwargs)
        if duplicates:
            raise DuplicateKeywordArgument("Duplicate keyword arguments: %s" % (",".join(duplicates)))
...
class Event(object):
    ...
    def fire(self, *args, **kwargs):
        for listener in list(self.listeners.values()):
            result_args = listener.callargs + CallArgs(args, kwargs)
            self.call_method(listener.fct, *result_args.args, **result_args.kwargs)
